...
if [[ "$ENV" != "local" && "$ENV" != "prod" ]]; then
    ...
    echo ""
    echo "   Environments:"
    echo "     local  — Local K3s with HTTP (gothicfire.local)"
    echo "     prod   — Production K3s with TLS (kornelian.com)"
    echo ""
    echo "   Flags:"
...
if [[ "$ENV" == "local" ]]; then
    ...
    echo "   1. Ensure DNS A record for kornelian.com → server IP"
    echo "   2. Open: https://kornelian.com"
    echo ""
    echo "   Check TLS certificate:"
    echo "   kubectl get certificate -n gothicfire"
    echo "   kubectl describe certificate kornelian-com-tls -n gothicfire"
fi
